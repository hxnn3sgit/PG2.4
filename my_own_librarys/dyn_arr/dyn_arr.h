...
// Iterator class:
/*
template<typename DynArr>
class dyn_arr_Iterator {

public:
	using ValueType = typename DynArr::ValueType;
...
public:
	DynArr() : data(nullptr), size(0), capacity(0) {}
	DynArr(int capacity) : data(new T[capacity]), capacity(capacity), size(0) {}
	...
	template<typename U>
	friend std::ostream& operator<<(std::ostream& out, const DynArr<U> &my_arr);
};
...
template<typename T>
const T DynArr<T>::at(const int index) const {
	if (index >= 0 && index < size) {
		// not out of bounds, okay
		return data[index];
	} else {
